...
const addNewCardToDOM = () => {
  ...
  const newCardElement = renderCards()[0]; // renderiza somente o novo card
  cards.insertBefore(newCardElement, cards.firstChild); // adiciona o novo card no início da lista de cards
};
...
const handleCardFormSubmit = (name, link) => {
  addNewCard(name, link); // Adiciona um novo card com o nome e link informados
  handlePopupCardAddToggle(); // Fecha o popup para adicionar novo card
  placeInputCardAdd.value = ""; // Limpa os campos do formulário
  imgLinkInputCardAdd.value = ""; // Limpa os campos do formulário
  allCards = getAllCards(); // atualiza a lista de todos os cards
  addNewCardToDOM(); // Atualiza a lista de todos os cards
  console.log("newCards:", newCards); // verificar se os novos cards estão sendo adicionados corretamente
  console.log("allCards:", allCards); // verificar se a lista de cards foi atualizada corretamente
};

// Esta função é chamada quando o formulário de adicionar novo card é submetido pelo usuário
const handleCardFormSubmitClick = (evt) => {
  evt.preventDefault(); // impede que a página seja recarregada após o envio do formulário
  const name = placeInputCardAdd.value; // recupera o valor do input de nome do local
  const link = imgLinkInputCardAdd.value; // recupera o valor do input de link da imagem
  // verifica se os inputs não estão vazios
  if (name && link) {
    handleCardFormSubmit(name, link); // chama a função que adiciona o novo card à lista
  }
};
